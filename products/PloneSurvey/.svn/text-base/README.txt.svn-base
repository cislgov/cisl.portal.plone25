...
Migrating from 1.0
  ...
  Existing surveys only had one question type - Survey Question
  To make sure these surveys still work with 1.1, you need to change the following line in the config.py file
  ADD_SURVEY_QUESTION = 0
  change 0 to 1, restart your server, and the existing surveys will continue to work

  The Survey Likert Question is also being deprecated, as this functionality will be added to
  Survey Select Question and Survey Matrix in future. Code is preserved in 1.1, as some users
  may be using it. To continue to use it, you need to change the following line in the config.py file
